...
def reshape_parts(partition_file):
    ...
    co_ma_df = pd.DataFrame(0, index=np.arange(len(parts)),columns = list(matrices.matrix))
    #make true or false df
    for co_nr,df in zip(parts.index.values.astype(int), parts):
...
#Dunn, Philippe
small_df = partitions[["Dunn2008","Philippe2009"]]
