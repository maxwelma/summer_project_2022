...
# cutoffs: chose to retain enough results to keep at least 1 hit for ~90% of partitions
    ...
    ava_df = read_blast_results('../reconciliation/blast/diamond_results/DP/*_permissive.txt',   #read only Dunn and Philippe) 
                                ava_colnames, 
                                ava_pident_cutoff, 
                                ava_evalue_cutoff)
    ...
    #ava_df_process_r[ava_df_process_r.qseqid.str.contains("Tribe1020")]
    ...
    paralogy_dict = {name: pd.DataFrame() for name in list_paralogy}
    for key in paralogy_dict.keys():
        df_temp = ava_df_process_r[ava_df_process_r.qseqid.str.contains(key)]
        #paralogy_dict[key] = df_temp
        if df_temp[df_temp.qcomp_nr != -1].empty:
            print(key+" missing component number")
        else:
            print(df_temp[df_temp.qcomp_nr != -1])
    
    list_contamination = ['Tribe321',
    'Tribe717',
    'Tribe858',
    'Tribe169',
    'Tribe325',
    ...
    cont_dict = {name: pd.DataFrame() for name in list_contamination}
    for key in cont_dict.keys():
        df_temp = ava_df_process_r[ava_df_process_r.qseqid.str.contains(key)]
        cont_dict[key] = df_temp
        if df_temp[df_temp.qcomp_nr != -1].empty:
            print(key+" missing component number")
        else:
            print(df_temp[df_temp.qcomp_nr != -1])
