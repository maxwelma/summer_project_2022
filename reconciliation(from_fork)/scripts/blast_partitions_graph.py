...
ava_df = read_blast_results('C:\\Users\\matil\\OneDrive\\Dokument\\GitHub\\animal_tree_root_fork\\add_new_data_test\\diamond_results\\dosReisDonoghueYang_2015_superalignment_permissive.txt', 
                            ava_colnames, 
                            ava_pident_cutoff, 
                            ava_evalue_cutoff)

# filter out self-matches
ava_df = ava_df[ava_df['qseqid'] != ava_df['sseqid']]       #Still includes missing partitions
...
ava_df = pd.concat((ava_qseq, ava_sseq, ava_df), axis=1) #Still includes missing partitions

# count matrix:partition <-> mscript:matrix:partition 
parts_groupby_columns = ['matrix1', 'part1', 'matrix2', 'part2']
parts_ava_df = ava_df[ava_df['part1'] != ava_df['part2']] #not including missing partitions, partition 1 not equal to partition 2, remove self hits(?)
parts = parts_ava_df.groupby(parts_groupby_columns).size().reset_index(name='count')
parts.to_csv('C:\\Users\\matil\\OneDrive\\Dokument\\GitHub\\animal_tree_root_fork\\add_new_data_test\\diamond_results/partitions_graph.tsv', sep='\t', index=False)
# count matrix:partition <-> mscript:matrix:partition 
taxon_groupby_columns = ['matrix1', 'taxon1', 'matrix2', 'taxon2']
taxa = ava_df.groupby(taxon_groupby_columns).size().reset_index(name='count')
taxa.to_csv('C:\\Users\\matil\\OneDrive\\Dokument\\GitHub\\animal_tree_root_fork\\add_new_data_test\\diamond_results/taxon_graph.tsv', sep='\t', index=False)

#################################### continue after doing busco and swissprot
